...
func (c SalesCreds) bayeuxUrl() string {
...
func GetSalesforceCredentials() SalesCreds {
	...
	params := url.Values{"grant_type": {"password"},
		"client_id":     {"3MVG9HxRZv05HarQRXBfFACyPOPOFwfoGaKrL.WRW1SYwaDJlB21edF83oUtwEPW6Ad95bQxCVliNFhQkyNx_"},
		"client_secret": {"25DC99B2D1D8881937E95796CFBDC14F3C6AA85E4E9591AE5B6D9B41B74FD251"},
		"username":      {"simon@vainu.io"},
		"password":      {"Onacci123"}}
	...
	var creds SalesCreds
	if err := decoder.Decode(&creds); err == io.EOF {
		logger.Fatal(err)
	} else if err != nil {
		logger.Fatal(err)
	} else if creds.AccessToken == "" {
		log.Printf("cred is %+v",creds)
...
func (b *Bayeux) TopicToChannel(creds SalesCreds, topic string) chan TriggerEvent {
	b.creds = creds
	err := b.getClientID()
	if err != nil {
